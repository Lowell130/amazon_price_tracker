<template>
  <section class="py-8 bg-white md:py-16 dark:bg-gray-900 antialiased rounded-t-lg">
      ...
      <div class="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16 px-3">
        ...
        <div class="mt-6 sm:mt-8 lg:mt-0 p-3">
          ...
          <div v-if="product.coupon_value" class="mt-2">
            <span class="inline-block bg-green-100 text-green-800 text-sm font-medium px-3 py-1 rounded dark:bg-green-900 dark:text-green-300">
    ...
    <!-- ✅ Modale iscrizione/disiscrizione email -->
    <div
      v-if="showModal"
    ...
    >
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-md">
        ...
        <p class="mb-2 text-sm text-gray-500 dark:text-gray-300">
          Inserisci la tua email per ricevere o cancellare le notifiche su questo prodotto.
        </p>
        <input
          v-model="email"
          type="email"
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:justify-end">
          <button
            ...
            class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:underline"
          >
            Chiudi
          </button>
          <button
            @click="subscribeToPriceDrop"
            class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg"
          >
            Iscriviti
          </button>
          <button
            @click="unsubscribeFromPriceDrop"
...
export default {
  ...
  methods: {
    ...
    async subscribeToPriceDrop() {
      ...
      try {
        const response = await fetch(`${process.env.VUE_APP_API_BASE_URL}/public/subscribe-alert`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error("Errore durante la registrazione all'alert.");
        }

        this.successMessage = "Iscrizione avvenuta con successo!";
        this.email = "";
    ...
    async unsubscribeFromPriceDrop() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(data.detail || "Errore durante la disiscrizione.");
        }

        this.successMessage = "Disiscrizione completata!";
        this.email = "";
        this.showModal = false;
      } catch (error) {
        this.errorMessage = error.message || "Errore durante la disiscrizione. Riprova.";
      }
    },
  },
};
